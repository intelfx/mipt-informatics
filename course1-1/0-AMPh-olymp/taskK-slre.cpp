...
/*
 * End SLRE library header (slre.h).
 */

static const size_t READ_BLOCK_SIZE = 4096;
static const size_t CAPTURES_COUNT = 64;

char* data = nullptr;
size_t data_allocd = 0;
size_t data_len = 0;
slre_cap captures[CAPTURES_COUNT];
...
bool regex_match (const char* regex);
bool regex_replace (const char* regex, const char* replace);

int main()
{
    // Matching against buffer begin is slow. Damned SLRE.
    data_append (" ");
    ...
    // Matching against buffer end is slow as well. Damned SLRE.
    data_append (" ");

    // Remove string literals. Damned SLRE, buggy as hell.
    while (regex_replace ("(\"[^\n\"]*\")", nullptr))
        ;
...
#define WORD_BOUNDARY "[\\[\\]\\(\\),\\s]"
#define WORD_REGEX(regex) ("(?i)" WORD_BOUNDARY regex WORD_BOUNDARY)

    if (regex_match (WORD_REGEX ("v?(ldr|str)"))) {
        // is ARM
        instruction_set_architecture = ISA_ARM;

        if (regex_match (WORD_REGEX ("(vldr|vstr)"))) {
            // has VFP insns
...
// Nested parentheses are not supported. Damned SLRE.
#define X86_BASE_REGISTER_REGEX "(ax|bx|cx|dx|si|di|sp|bp)"
#define X86_GP_REGISTER_REGEX "[re]?" X86_BASE_REGISTER_REGEX
#define X86_SEGMENT_REGISTER_REGEX "(cs|ds|ss)"

        if (regex_match (WORD_REGEX ("%" X86_GP_REGISTER_REGEX)) ||
            regex_match (WORD_REGEX ("%" X86_SEGMENT_REGISTER_REGEX))) {
            // has AT&T-style registers
            x86_flavor = X86_FLAVOR_ATT;
        }

        if (regex_match (WORD_REGEX ("(%?)r" X86_BASE_REGISTER_REGEX))) {
            // has 64-bit registers
            x86_operands = X86_OPERAND_64;
        } else if (regex_match (WORD_REGEX ("%?e" X86_BASE_REGISTER_REGEX))) {
            // has 32-bit registers
            x86_operands = X86_OPERAND_32;
        }

        if (regex_match (WORD_REGEX ("finit|fld|fst"))) {
            // has x87 FPU insns
...
bool regex_match (const char* regex)
{
    int result = slre_match (regex, data, data_len, captures, CAPTURES_COUNT);

    if (result < -1) {
        fprintf (stderr, "regex '%s' returned %d\n", regex, result);
        abort();
    }
...
bool regex_replace (const char* regex, const char* replace)
{
    ...
    if (!regex_match (regex)) {
        return false;
    }

    size_t dest_len = data_len - captures[0].len + replace_len;
    char* dest = (char*) malloc (dest_len + 1);

    size_t match_pos = captures[0].ptr - data;
    if (match_pos) {
        memcpy (dest, data, match_pos);
    }

    if (replace_len) {
        memcpy (dest + match_pos, replace, replace_len);
    }

    if (data_len > match_pos + captures[0].len) {
        memcpy (dest + match_pos + replace_len, data + match_pos + captures[0].len, data_len - (match_pos + captures[0].len));
    }
...
/*
 * End SLRE library source (slre.c).
 */
